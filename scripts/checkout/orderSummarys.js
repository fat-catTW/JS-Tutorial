...
export function renderOrderSummary(){
  ...
  document.querySelectorAll('.js-save-link').forEach((link) => {
    link.addEventListener('click', () => {
      ...
      let productAmount = Number(document.querySelector(`.js-quantity-input-${productId}`).value);
      if(productAmount < 0 || productAmount > 1000)
      {
        alert('Number of the product should be within 0 ~ 1000\nNumber is reset to 1.');
        productAmount = 1;
      }

      updateQuantity(productId, productAmount);
      renderCheckoutHeader(updateCartQuantity());
      document.querySelector(`.js-quantity-label-${productId}`).innerHTML = productAmount;
  ...
  document.querySelectorAll('.quantity-input').forEach((link) => {
    link.addEventListener('keypress', (event) => {
      if(event.key === 'Enter')
      {
        ...
        updateQuantity(productId, productAmount);
        document.querySelector('.js-checkout-items-num').innerHTML = `${updateCartQuantity()} items`;
        document.querySelector(`.js-quantity-label-${productId}`).innerHTML = productAmount;
        container.classList.remove('is-editing-quantity');
      }
